document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchData() {
    try {
      const response = await fetch(
        "http://localhost:5001/api/v1/buildings/1/occupied-sessions"
      );
  ...
  async function updateAcceptedList() {
    ...
    const acceptedListContainer = document.getElementById("adminOccupiedLists");
    ...
    acceptedSessions.forEach((session) => {
      ...
      customCard.innerHTML = `
          <div class="custom-card">
                     ...
                     B Book Time: ${session.start_time} <br />
